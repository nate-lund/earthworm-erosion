...
rusle2 <- project(rusle2, landcover) %>% # reproject rusle to match landcover
  resample(landcover, method = "bilinear") # resample slope to match resolution
...
#' [TERRA] ---------------------------------------------------------------
# generate tables and plots for each landcover ----


# define rusle2 bins & create a maxtrix of the bin distribtions
bins <- data.frame(A = c(-10, 0, 0.5, 1, 2, 5), # "left" bin side
                   B = c(0, 0.5, 1, 2, 5, 100), # "right" bin side
                   C = c(1, 2, 3, 4, 5, 6))
...
slope.cats <- data.frame(
  ID = c(1, 2, 3, 4, 5, 6),
  slope_class = c("< 0", "0 - 0.5", "0.5 - 1", "1 - 2", "2 - 5", "5 - 100")
)
...
# export
write.csv(stack.vis, hert("_analysis/rusle2-landcover-bins.csv")) # export clean table for manuscript
write.csv(stack.sum, hert("_analysis/rusle2-landcover-bins-plots.csv")) # export df to plot later
